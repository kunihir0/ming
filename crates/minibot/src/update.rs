...
#[poise::command(slash_command, required_permissions = "ADMINISTRATOR", category = "System")]
pub async fn update_bot(ctx: PoiseContext<'_>) -> Result<(), Error> {
    ...
    ctx.say(format!("Checking for updates from {}/{}...", repo_owner, repo_name)).await?;
    ...
    match status {
        self_update::Status::Updated(v) => {
            ctx.say(format!("✅ Successfully updated to version `{}`! Please restart the bot process.", v)).await?;
            info!("Bot updated to version {}. Requires restart.", v);
        }
        self_update::Status::UpToDate(_v) => {
            ctx.say("✅ The bot is already up-to-date or no new release was found.").await?;
        }
    }

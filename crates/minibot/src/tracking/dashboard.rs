...
pub async fn refresh_dashboard(
    ...
    // Unassigned players
    let mut unassigned: Vec<&db::models::TrackedPlayer> =
        players.iter().filter(|p| p.group_id.is_none()).collect();
    ...
    for group in groups {
        let mut group_players: Vec<&db::models::TrackedPlayer> = players
            .iter()
    ...
    match action {
        "addperson" => {
            let modal = CreateModal::new(
                format!("track_addperson_modal_{}", server_id),
                "Add Person to Track",
            )
        ...
        "removeperson" => {
            let modal = CreateModal::new(
                format!("track_removeperson_modal_{}", server_id),
                "Remove Person",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(serenity::InputTextStyle::Short, "Steam ID 64", "steam_id")
        ...
        "assign" => {
            let modal = CreateModal::new(
                format!("track_assign_modal_{}", server_id),
                "Assign to Group",
            )
            .components(vec![
        ...
        "creategroup" => {
            let modal = CreateModal::new(
                format!("track_creategroup_modal_{}", server_id),
                "Create Group",
            )
            .components(vec![CreateActionRow::InputText(
        ...
        "deletegroup" => {
            let modal = CreateModal::new(
                format!("track_deletegroup_modal_{}", server_id),
                "Delete Group",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(serenity::InputTextStyle::Short, "Group Name", "group_name")
        ...
        "checkhours" => {
            let modal = CreateModal::new(
                format!("track_checkhours_modal_{}", server_id),
                "Check Tracked Player Hours",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(serenity::InputTextStyle::Short, "Steam ID 64", "steam_id")
                    .placeholder("7656119...")
        ...
        "checkatlas" => {
            let modal = CreateModal::new(
                format!("track_checkatlas_modal_{}", server_id),
                "Check Atlas Stats",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(serenity::InputTextStyle::Short, "Steam ID 64", "steam_id")
        ...
        "analytics" => {
            let modal = CreateModal::new(
                format!("track_analytics_modal_{}", server_id),
                "Player Analytics",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(serenity::InputTextStyle::Short, "Steam ID 64", "steam_id")
        ...
        "getbmid" => {
            let modal = CreateModal::new(
                format!("track_getbmid_modal_{}", server_id),
                "Get Linked BM ID",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(serenity::InputTextStyle::Short, "Steam ID 64", "steam_id")
        ...
        "clearaliases" => {
            let modal = CreateModal::new(
                format!("track_clearaliases_modal_{}", server_id),
                "Clear Aliases",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(serenity::InputTextStyle::Short, "Steam ID 64", "steam_id")
        ...
        "clearall" => {
            let modal = CreateModal::new(
                format!("track_clearall_modal_{}", server_id),
                "Clear All Data",
            )
            .components(vec![CreateActionRow::InputText(
                CreateInputText::new(
                    serenity::InputTextStyle::Short,
                    "Type CONFIRM to erase everything",
                    "confirm_text",
                )
                .placeholder("CONFIRM")
                .required(true),
...
pub async fn handle_modal(
    ...
    match action {
        ...
        "assign" => {
            if let (Some(steam_id), Some(group_name)) =
                (get_input("steam_id"), get_input("group_name"))
            {
        ...
        "deletegroup" => {
            if let Some(group_name) = get_input("group_name") {
                    ...
                    .first::<db::models::TrackGroup>(&mut conn)
                {
                    diesel::update(
                        tracked_players::dsl::tracked_players
                            .filter(tracked_players::dsl::group_id.eq(group.id)),
                    )
                    .set(tracked_players::dsl::group_id.eq(None::<i32>))
                    .execute(&mut conn)?;
                    diesel::delete(
                        track_groups::dsl::track_groups.filter(track_groups::dsl::id.eq(group.id)),
                    )
                    .execute(&mut conn)?;
                } else {
                    success_msg = format!("Group '{}' not found.", group_name);
        ...
        "clearaliases" => {
            if let Some(steam_id) = get_input("steam_id") {
                ...
                {
                    diesel::delete(
                        player_name_history::dsl::player_name_history
                            .filter(player_name_history::dsl::tracked_player_id.eq(player.id)),
                    )
                    .execute(&mut conn)?;
                } else {
            ...
            if let Some(confirm) = get_input("confirm_text") {
                if confirm.trim() == "CONFIRM" {
                    ...
                    for p in players {
                        diesel::delete(
                            player_name_history::dsl::player_name_history
                                .filter(player_name_history::dsl::tracked_player_id.eq(p.id)),
                        )
                        .execute(&mut conn)?;
                    }
                    ...
                    // Delete groups
                    diesel::delete(
                        track_groups::dsl::track_groups
                            .filter(track_groups::dsl::server_id.eq(server_id)),
                    )
                    .execute(&mut conn)?;

                    success_msg =
                        "✅ All players, aliases, and groups have been permanently deleted."
                            .to_string();
                } else {
        ...
        "checkhours" => {
            if let Some(steam_id) = get_input("steam_id") {
                    ...
                    .first::<db::models::TrackedPlayer>(&mut conn)
                {
                    ...
                    if let Ok(hours_text) =
                        crate::tracking::hours_cmd::get_player_hours_text(db_pool, steam_id, bm_id)
                            .await
                    {
                    ...
                    success_msg =
                        "❌ Player is not tracked on this dashboard. Use 'Add Person' first."
                            .to_string();
                }
            }
        }
            ...
            if let Some(steam_id) = get_input("steam_id") {
                    ...
                    .first::<db::models::TrackedPlayer>(&mut conn)
                {
                    match crate::tracking::atlas::client::AtlasClient::new() {
                                ...
                                Ok(res) => {
                                    if let Some(ap) = res.player {
                                        ...
                                        if let Some(atlas_bm_id) = ap.bm_player_id {
                                            ...
                                            let _ = diesel::update(
                                                tracked_players::dsl::tracked_players
                                                    .filter(tracked_players::dsl::id.eq(player.id)),
                                            )
                                            .set(
                                                tracked_players::dsl::bm_player_id
                                                    .eq(Some(&atlas_bm_str)),
                                            )
                                            .execute(&mut conn);
                                            let _ = db::upsert_player_link(
                                                &mut conn,
                                                &steam_id,
                                                &atlas_bm_str,
                                            );
                                        ...
                                        if let Some(b) = ap.bm_player_id {
                                            txt.push_str(&format!("* BattleMetrics ID: {}\n", b));
                                        }
                                        if let Some(ls) = ap.last_online {
                                            txt.push_str(&format!("* Last Online: {}\n", ls));
                                        }
                                        txt.push_str(&format!(
                                            "\n[Hours]\n* Atlas Hours: {}\n* BM Hours: {} hrs\n",
                                            ap.atlas_hours,
                                        ...
                                        success_msg =
                                            "❌ Player not found on Atlas Rust.".to_string();
                                    }
                                }
                                Err(e) => {
                        ...
                        Err(_) => {
                            success_msg =
                                "❌ Atlas API token not configured in .env (ATLAS_JWT_TOKEN)."
                                    .to_string();
                        }
                    }
                } else {
        ...
        "analytics" => {
            if let Some(steam_id) = get_input("steam_id") {
                if let Ok(player) = tracked_players::dsl::tracked_players
                    ...
                    .first::<db::models::TrackedPlayer>(&mut conn)
                {
                    match crate::tracking::analytics::report::get_player_analytics(
                        db_pool, player.id,
                    ) {
                        Ok(data) => {
                            let mut embed = serenity::builder::CreateEmbed::new()
                                .title(format!(
                                    "Analytics: {}",
                                    player.last_known_name.as_deref().unwrap_or("Unknown")
                                ))
                                .color(0x5865F2)
                                .field(
                                    "Total Playtime",
                                    format!("{:.1} hrs", data.total_hours),
                                    true,
                                )
                                .field("Sessions", data.session_count.to_string(), true)
                                .field(
                                    "Avg Session",
                                    format!("{:.0} mins", data.avg_session_mins),
                                    true,
                                );
                            ...
                            if let Ok(chart_bytes) =
                                crate::tracking::analytics::charts::generate_activity_chart(
                                    &data.daily_playtime,
                                )
                            {

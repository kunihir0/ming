...
pub static ITEMS: LazyLock<HashMap<String, ItemDefinition>> = LazyLock::new(|| {
    ...
    let raw_items: Vec<RawItemDefinition> = serde_json::from_str(json).expect("Failed to parse items.json");
    
    let mut map = HashMap::new();
    for raw in raw_items {
        map.insert(raw.id.to_string(), ItemDefinition {
...
pub fn search_items_smart(query: &str) -> Vec<(i32, String, String)> {
    ...
    let is_regex = query.contains('*') || query.contains('^') || query.contains('$') || query.contains('[');

...
async fn main() {
    ...
    let (len, _) = timeout(Duration::from_secs(3), socket.recv_from(&mut buf)).await.unwrap().unwrap();
    ...
    if data[4] == 0x41 {
        ...
        socket.send_to(&req, addr).await.unwrap();
        let (len2, _) = timeout(Duration::from_secs(3), socket.recv_from(&mut buf)).await.unwrap().unwrap();
